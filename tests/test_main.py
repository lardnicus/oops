...
from oops import main, conf, types
...
def test_get_command():
    with patch('oops.main.Popen') as Popen, \
            patch('oops.main.os.environ',
                  new_callable=lambda: {}), \
            patch('oops.main.wait_output',
                  return_value=True):
        Popen.return_value.stdout.read.return_value = b'stdout'
        Popen.return_value.stderr.read.return_value = b'stderr'
        assert main.get_command(Mock(), ['oops', 'apt-get',
                                         'search', 'vim']) \
...
def test_confirm(capsys):
    ...
    with patch('oops.main.sys.stdin.read', return_value='\n'):
        assert main.confirm('command', Mock(require_confirmation=True,
                                            no_colors=True))
        assert capsys.readouterr() == ('', 'command [enter/ctrl+c]')
    # When confirmation required and ctrl+c:
    with patch('oops.main.sys.stdin.read', side_effect=KeyboardInterrupt):
