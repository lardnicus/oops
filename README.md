...
[On in your shell config (Bash, Zsh, Fish, Powershell).](https://github.com/nvbn/theoops/wiki/Shell-aliases)
...
Oops! tries to match rule for the previous command, create new command
...
in `~/.theoops/rules`. Rule should contain two functions:
`match(command: Command, settings: Settings) -> bool`
and `get_new_command(command: Command, settings: Settings) -> str`.

`Command` has three attributes: `script`, `stdout` and `stderr`.

`Settings` is a special object filled with `~/.theoops/settings.py` and values from env, [more](#settings).
...
Oops! has a few settings parameters, they can be changed in `~/.theoops/settings.py`:
